...
export function useShareCard() {
	...
	const shareCard = useCallback(async (element: HTMLDivElement | null) => {
		if (!element) {
		...
		try {
			...
			canvas.toBlob(async (blob) => {
				...
				const file = new File([blob], 'who-sings-score.png', { type: 'image/png' });
			...
			try {
				if (navigator.clipboard) {
					...
					alert('Unable to generate image. Link copied to clipboard instead!');
				} else {
					alert('Unable to share. Please take a screenshot to share your score!');
				}
